...
class ParentProfile(db.Model):
    ...
    children = db.relationship(
        ...
        secondaryjoin=ParentChildLink.student_id == StudentProfile.id,
        backref='parents',
        lazy='dynamic'
    )
...
class ClassFeeStructure(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('class_level', 'academic_year', 'semester', 'description', name='uq_class_fee_group'),)
...
class StudentFeeBalance(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('student_id', 'academic_year', 'semester', name='uq_student_fee_balance'),)
...
class CourseAssessmentScheme(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("course_id", "teacher_id", name="uix_course_teacher_scheme"),)
