...
def add_exam():
    ...
    form.programme_name.choices = [('', '— Select Programme —')] + get_programme_choices()

    # Populate level choices
    form.programme_level.choices = [('', '— Select Level —')] + get_level_choices()

    # Populate course choices for the selected programme and level
    ...
    if selected_programme and selected_level:
        ...
        form.course_id.choices = [(0, '— Select Course —')] + [(c.id, f"{c.code} — {c.name}") for c in courses]
    else:
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            current_app.logger.warning(f"Exam notification failed: {e}")
        
        flash("✓ Exam created and students notified.", "success")
...
def edit_exam(exam_id):
    ...
    # Populate programme choices from helper
    form.programme_name.choices = [('', '— Select Programme —')] + get_programme_choices()
    
    # Populate level choices from helper
    form.programme_level.choices = [('', '— Select Level —')] + get_level_choices()
...
def edit_exam_question(exam_id, question_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.commit()
            current_app.logger.info(f"Question {question_id} updated")
    ...
    # GET: Prepare initial values for template
    options = []
    math_answers = []
    tf_choice = None
    rubric = ""
    ...
    if question.question_type == 'mcq':
        ...
        for o in opts:
            if o.text.lower().startswith('true') and o.is_correct:
                tf_choice = 'true'
            if o.text.lower().startswith('false') and o.is_correct:
                tf_choice = 'false'
